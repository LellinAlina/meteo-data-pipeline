...
REGION_FILTER = "Иркутская область"  # Название региона, по которому фильтруем
...
# ───── Фильтрация по Иркутской области ─────
cities_irkutsk = [city for city in cities if city.get("region") == REGION_FILTER]

if not cities_irkutsk:
    raise RuntimeError("❌ В таблице city нет городов из Иркутской области.")

print(f"🔍 Найдено городов в Иркутской области: {len(cities_irkutsk)}")
